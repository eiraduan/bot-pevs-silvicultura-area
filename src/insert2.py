...
def main():
    ...
    TABELA_DESTINO = "dados_pevs_silvicultura_area2"
    ...
    if not arquivo_consolidado.exists():
        ...
        try:
            ...
            df.rename(columns={
                ...
                "Variável": "variavel",
                "Espécie florestal (Código)": "especie_florestal_codigo",
                "Espécie florestal": "especie_florestal"
            }, inplace=True)

            df['valor'] = df['valor'].replace({'','-','..','...'}, pd.NA)

            outras_colunas = [col for col in df.columns if col != 'id']

            nova_ordem = ['id'] + outras_colunas
